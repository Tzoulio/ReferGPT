...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'ROOT_DIR': os.path.join(code_path, 'data/'),  # Location of dataset
    ...
    def __init__(self, config=None):
        ...
        self.gt_fol = os.path.join(self.root_dir , self.config['GT_FOLDER'], split_fol)
        self.tracker_fol = os.path.join(self.root_dir , self.config['TRACKERS_FOLDER'], split_fol)
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    ...
                    curr_file = os.path.join(self.config["ROOT_DIR"], self.tracker_list[0], seq.split('+')[0], seq.split('+')[1], 'predict.txt')
                    if not os.path.isfile(curr_file):
                        # f = open(curr_file, 'a')
                        # f.close()
                        print('Tracker file not found: ' + curr_file)
    ...
    def _get_seq_info(self):
        ...
        if self.config["SEQ_INFO"]:
            ...
            if self.config["SEQMAP_FILE"]:
                seqmap_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), self.config["SEQMAP_FILE"])
            else:
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            ...
            if is_gt:
                ...
                file = os.path.join(self.config["ROOT_DIR"],self.tracker_list[0], seq.split('+')[0], seq.split('+')[1], 'predict.txt')
