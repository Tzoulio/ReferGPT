...
class Trajectory:
    ...
    def init_trajectory(self):
        ...
        detected_state_template = np.asmatrix(detected_state_template).T
        update_covariance_template = np.asmatrix(update_covariance_template).T
    ...
    def init_parameters(self):
        ...
        :return:
        """
        self.A = np.asmatrix(np.eye(self.track_dim))
        self.Q = np.asmatrix(np.eye(self.track_dim))*self.config.state_func_covariance
        self.P = np.asmatrix(np.eye(self.track_dim-6))*self.config.measure_func_covariance
        self.B = np.asmatrix(np.zeros(shape=(self.track_dim-6,self.track_dim)))
        self.B[0:3,:] = self.A[0:3,:]
        self.B[3:,:] = self.A[9:,:]

        self.velo = np.asmatrix(np.eye(3))*self.scanning_interval
        self.acce = np.asmatrix(np.eye(3))*0.5*self.scanning_interval**2
        ...
        self.H = self.B.T
        self.K = np.asmatrix(np.zeros(shape=(self.track_dim,self.track_dim)))
    ...
    def state_update(self,
        ...
        updated_covariance = (np.asmatrix(np.eye(self.track_dim)) - KF_gain*self.B)*predicted_covariance
        ...
        current_ob.similarity = similarity
        current_ob.similarity_score=similarity_score
    ...
    def filtering(self, config):
        """
        Filter the trajectory in a global or near online way, using config-driven parameters.
        Args:
            config: configuration object with all relevant parameters
        """
        wind_size = int(config.LiDAR_scanning_frequency * config.latency)
        if wind_size < 0:
            detected_num = 1e-5
            score_sum = 0
            detected_num_similarity = 1e-6
            tot_sim = 1e-8
            total_sim_score = 1e-5
            for key in self.trajectory.keys():
                ob = self.trajectory[key]
                if ob.similarity is not None:
                    tot_sim += 1
                    if ob.similarity:
                        detected_num_similarity += 1
                    if ob.similarity_score is not None:
                        total_sim_score += ob.similarity_score
            ...
            real_sim = detected_num_similarity / tot_sim
            average_similarity_score = total_sim_score / detected_num_similarity
            for key in self.trajectory.keys():
                ob = self.trajectory[key]
                ob.score = score
            for key in self.trajectory.keys():
                ob = self.trajectory[key]
                if self.first_updated_timestamp - 1 <= key <= self.last_updated_timestamp and ob.similarity_score is None:
                    ob.similarity_score = average_similarity_score
        else:
