...
def compute_bounding_boxes(P2, corners3d):
    """
    :param corners3d: (N, 8, 3) corners in rect coordinate
    :param P2: (3, 4) projection matrix
    :return: boxes: (None, 4) [x1, y1, width, height] in rgb coordinate
    :return: boxes_corner: (None, 8) [xi, yi] in rgb coordinate
    """
    sample_num = corners3d.shape[0]
    corners3d_hom = np.concatenate((corners3d, np.ones((sample_num, 8, 1))), axis=2)  # (N, 8, 4)
    ...
    # Concatenate to form [x1, y1, width, height]
    img_boxes = np.concatenate((x1.reshape(-1, 1), y1.reshape(-1, 1), 
                                width.reshape(-1, 1), height.reshape(-1, 1)), axis=1)
    boxes_corner = np.concatenate((x.reshape(-1, 8, 1), y.reshape(-1, 8, 1)), axis=2)

    # Clip bounding box coordinates to image dimensions
    img_boxes[:, 0] = np.clip(img_boxes[:, 0], 0, 1242 - 1)  # Clip x1
    img_boxes[:, 1] = np.clip(img_boxes[:, 1], 0, 375 - 1)   # Clip y1
    img_boxes[:, 2] = np.clip(img_boxes[:, 2], 0, 1242 - 1)  # Clip width
    img_boxes[:, 3] = np.clip(img_boxes[:, 3], 0, 375 - 1)   # Clip height

    return img_boxes, boxes_corner

def corners3d_to_img_boxes_torch(P2, corners3d):
    """
...
def bb3d_2_bb2d(bb3d,P2):
    ...
    box, _ = compute_bounding_boxes(P2, pts[:, :, 0:3])
...
def bb3d_2_bb2d_torch(bb3d, P2):
    ...
    box, _ = corners3d_to_img_boxes_torch(P2, pts[:, :, :3])

    return box


def display_image_with_bboxes( image, detections_box_2d):
            """
            Displays an image with 2D bounding boxes overlaid.
            
            Args:
                image_path (str): Path to the image file.
                detections_box_2d (torch.Tensor): Tensor of bounding boxes in the format
            ...
            ax.set_title("Image with 2D Bounding Boxes")
            ax.axis("off")
